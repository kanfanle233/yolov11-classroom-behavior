...
def sample_keys_jsonl(path: Path, n=3):
    samples = []
...
def main():
    ...
    if not objects_path.exists():
        ...
        dets = 0
        bad = 0
        for obj in iter_jsonl(objects_path):
            lines += 1
    ...
    if not actions_path.exists():
        ...
        print("  samples keys:", sample_keys_jsonl(actions_path, n=3))
        ...
        first = None
        for obj in iter_jsonl(actions_path):
            if "__bad_line__" in obj:
                continue
            if first is None:
                first = obj
    ...
    if not transcript_path.exists():
        ...
        print("  samples keys:", sample_keys_jsonl(transcript_path, n=3))
        lines = 0
        ok = 0
        for obj in iter_jsonl(transcript_path):
            if "__bad_line__" in obj:
                continue
            lines += 1
