...
def resolve_paths(custom_index_path=None, custom_out_root=None):
    """
    解析项目核心路径
    基于当前脚本位置：YOLOv11/scripts/intelligence class/02_batch_run.py
    """
    ...
    # 回退逻辑：如果脚本位置变化，向上找含 data 的目录
    if not (project_root / "data").exists():
        probe = current_file.parent
        while not (probe / "data").exists() and len(probe.parts) > 1:
            probe = probe.parent
        project_root = probe

    # 单视频执行器：01_run_single_video.py (要求在同一目录)
    target_script = current_file.parent / "01_run_single_video.py"
